...
- Frontend: React 19 · Vite 7 · TypeScript · React Router v7 · Tailwind v4 · @dnd-kit · xterm.js · zustand · Sonner · React Hook Form
...
并发控制：全局 `maxConcurrentTickets`（默认 3），所有项目共享。由 cc/manager.ts 管理进程生命周期、队列调度、健康监控、崩溃 `--resume` 重启。优先级调整不触发抢占（不杀运行中的 CC），下次有空 slot 时按新 priority 排序启动。
...
| `maxConcurrentTickets` | 3 | 最大并发 Ticket CC 数，全局共享。调低时不杀已有进程，等自然降到新上限以下 |
